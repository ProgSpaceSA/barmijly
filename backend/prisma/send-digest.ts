...
function preview(digest: Awaited<ReturnType<DigestService['buildDigest']>>) {
  ...
  console.log(
    `  actions=${actionTotal}  meetings=${digest.todayMeetings.length}  mentions=${digest.mentions.length}  unread=${digest.unreadTotal}  bugs=${digest.bugAlertTotal}  tasks=${digest.openTasks.length}  overdue=${digest.overdue.length}  dueSoon=${digest.dueSoon.length}`,
  );
